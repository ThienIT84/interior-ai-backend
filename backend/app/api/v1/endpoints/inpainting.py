...
from app.core.job_executor import JobExecutor, get_job_executor
...
# Job storage now uses Redis via JobService (persistent across restarts)
...
async def _process_inpainting_job(
    job_id: str,
    request: InpaintRequest,
    ...
    try:
        ...
        job_service.update_job(job_id, progress=0.15)
        image = Image.open(image_files[0]).convert("RGB")
        mask_raw = Image.open(mask_files[0]).convert("L")
        ...
        mask_ready = prep.mask
        job_service.update_job(job_id, progress=0.25)
        ...
        inpainting = get_inpainting_service()
        job_service.update_job(job_id, progress=0.30)
        ...
        job_service.update_job(job_id, progress=0.80)
        ...
        job_service.update_job(job_id, progress=0.95)
